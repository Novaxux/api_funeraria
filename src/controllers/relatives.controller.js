...
export const getRelativeById = async (req, res) => {
  try {
    ...
    const relative = await relativesRepo.getRelativeIfLinked(
      id_user,
      id_relative
    );
    if (!relative)
      return res
        .status(404)
        .json({ message: "Relative not found or not linked to this user." });
    ...
    return res
      .status(200)
      .json({ relative: { id_relative: rid, name, relationship, age } });
  } catch (error) {
