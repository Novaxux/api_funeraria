...
export const createMemory = async (req, res) => {
  try {
    ...
    try {
      ...
      await memoriesRepo.linkMemoryToRelatives(connection, id_memory, relativeIds);

      await connection.commit();
      connection.release();

      return res.status(201).json({ message: "Memory saved successfully.", id_memory, relativeIds });
    } catch (err) {
...
export const editMemory = async (req, res) => {
  try {
    ...
    if (!id_memory) return res.status(400).json({ message: "Memory ID is required." });

    const connection = await pool.getConnection();
    try {
      await connection.beginTransaction();

      await memoriesRepo.updateMemory(connection, id_memory, { title, content });

      // If relatives provided, replace links
      if (Array.isArray(relativeIds)) {
        await memoriesRepo.replaceMemoryRelatives(connection, id_memory, relativeIds);
      }

      await connection.commit();
      connection.release();

      return res.status(200).json({ message: "Memory updated successfully.", relativeIds });
    } catch (err) {
      await connection.rollback();
      connection.release();
      throw err;
